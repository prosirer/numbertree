...
The same tables for English are used for French birth names. 
...
### <span id="anchor-11"></span>Romanian
...
### <span id="anchor-12"></span>Hungarian
...
### <span id="anchor-30"></span>Indonesian (Malay)
...
### <span id="anchor-35"></span>Mongolian
...
## <span id="anchor-38"></span>Indian Languages
...
### <span id="anchor-41"></span>Punjabi
...
### <span id="anchor-51"></span>Burmese
...
### <span id="anchor-52"></span>Tibetan

This script follows the Llasa variant. This script works likewise for
the Dzongkha language.

The Tibetan language is an abugida, and so each consonant contains an
inherent vowel unless otherwise noted. The inherent vowel for this
language is considered the last “letter” of the alphabet.

The inherent vowel of ཨ is used in the following manner. If it is
written by itself, then it has a value according to its position at the
end of the consonants on the table. However, if it is used as the
inherent vowel, then its value is equal to 1. 

Any vowel modifiers do not change the original value of the vowel.

If the srog med or ྄ is used, then the inherent vowel is not counted
with the consonant. 

For consonant clusters, the values of each consonant are added together
along with the vowel of the original consonant. 

Tibetan – Lightning Chart

|            |   |   |   |   |   |   |   |   |   |   |
| ---------- | - | - | - | - | - | - | - | - | - | - |
| 1          | 2 | 3 | 4 | 5 | 6 | 7 | 8 | 9 | ↊ | ↋ |
| ཀ          | ཁ | ག | ང | ཅ | ཆ | ཇ | ཉ | ཏ | ཐ | ད |
| ན          | པ | ཕ | བ | མ | ཙ | ཚ | ཛ | ཝ | ཞ | ཟ |
| འ          | ཡ | ར | ལ | ཤ | ས | ཧ | ཨ |   |   |   |
| (inherent) | ི | ུ | ེ | ོ |   |   |   |   |   |   |

Tibetan – Growth Chart
...
## <span id="anchor-53"></span>African Languages

These languages are typically based off of the Latin script due to the
influence from the colonial era. Many languages have a low literacy rate
and are primarily spoken. To divinate in such instances, finding someone
who knows how to use the letters of a language is ideal, then
pronouncing the name as accurately as possible. 

### <span id="anchor-54"></span>Amazigh (Berber)
...
### <span id="anchor-57"></span>Swahili
...
### <span id="anchor-72"></span>Lingala
...
### <span id="anchor-73"></span>Malagasy
...
## <span id="anchor-75"></span>Middle Eastern Languages

Many of these languages are abjad, which use consonants as the main
letters with inferred vowels (not written). Most of these abjad
languages are considered modified as the vowels are written with
diacritics. As long as the birthname is written with these vowel marks,
then divination in the native language is possible. Otherwise, unless
other options are available, a birth name as given within an abjad
language must be *transcripted* (not translated) into a Latin alphabet
to capture the entire phonology of the name. This ensures the vowel
structure is properly accounted for. 

Oftentimes, there are two birth certificates for a name, one in the
local language and the other a Latinized form. In such cases, the one in
the local language is always used as the primary. This is due to
(former) colonial powers failing to represent all the sounds in a
language, as such only giving an approximation. If no birth certificate
exists, then the spoken name in the dialect given at birth is written
down as accurately as possible. 

If the birth certificate in the local language also uses the calendar
other than Gregorian for a given area (such as Iran or Afghanistan),
then the Morphic Field must be calculated with this calendar. Please see
...
### <span id="anchor-78"></span>Persian
...
### <span id="anchor-80"></span>Turkish
...
## <span id="anchor-83"></span>American Languages

Much of the early Mesoamerican writing systems were logographic in
structure, such as the Mayan hieroglyphs. As such, these early systems
will not work with the system in this book.

However, in modern times, native languages are typically either spoken
using Latin letters or with a syllabic structure. 

### <span id="anchor-84"></span>Nahuatl
...
### <span id="anchor-85"></span>K’iche’ \[WIP\]

### <span id="anchor-86"></span>Quechua
...
### <span id="anchor-90"></span>Guarani \[WIP\]

### <span id="anchor-91"></span>Aymara \[WIP\]

## <span id="anchor-92"></span>Other Languages

Other languages that do not fit a wide geographical region can be found
in this section.
